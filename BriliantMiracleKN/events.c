...
PCREATE_PROCESS_NOTIFY_ROUTINE_EX ProcessNotifyCallbackEx(HANDLE parentId, HANDLE processId, PPS_CREATE_NOTIFY_INFO notifyInfo)
{
	...
	if (notifyInfo)
	{
		...
		if (wcsstr(notifyInfo->ImageFileName->Buffer, L"\\TecentPhone.exe"))
		{
	...
	else
	{
		...
		else if (TkfID == ProcID)
		{
			DebugMessageNormal("FFT Shutdown detected, zeroing addresses. %d\r\n", TkfID);
			TkfID = 0;
			UnityPlayerDll = 0;
			UnityPlayerDllSize = 0;
		}
	}
